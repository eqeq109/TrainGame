...
export default class Bomb extends ZepetoScriptBehaviour {
    ...
    private *SetAnimation() {
        ...
        //for SFX check
        let sfxCheck:bool = false;

        while (timer < this.animationTime) {
            timer += Time.deltaTime;
            let timeRatio: float = timer / this.animationTime;
            this.objectBomb.localPosition = new UnityEngine.Vector3(
                easing.inOutBounce(startPosition.x, 0, timeRatio),
